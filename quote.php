...
$p_amt = mysqli_query($con, "SELECT * FROM prem WHERE key='" . $_POST['key1'] . "'");
$amt_tt = mysqli_query($con, "SELECT * FROM twitterdb");
$cue = ($p_amt->num_rows > 0) ? $amt_tt->num_rows : "50";
$amt_t = ($_POST['spinnerr'] > $cue) ? "50" : $_POST['spinnerr'];
...
$url1 = parse_url($_POST['url2'], PHP_URL_PATH);
$url = explode("/", $url1);
$text_array = explode("|", $_POST['additionaltext']);
...
$usr = (array) $acc1[0];
  $c->post('statuses/update', array('status' => "“@".$url[1].": ".$usr['text']."” ".$text, 'in_reply_to_status_id' => $url[3]));
$ff++; 
}
